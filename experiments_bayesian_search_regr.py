...
executions_per_trial = 5 #number of trainings and evaluations to average on the same combination of params
...
last_results_filename = os.path.join(last_results_path, 'Last Experiment Best Results Comparison regr.txt')
last_results_logger = open(last_results_filename, 'w')

#datasets = ['FloodModeling1','FloodModeling2','Covid3Month']
datasets = ['FloodModeling1','FloodModeling2','FloodModeling3','Covid3Month','IEEEPPG', 'AppliancesEnergy','NewsHeadlineSentiment']  #new datasets not tried in any related experiments before

time_string_start = strftime("%H.%M.%S", localtime())
results_comparison_path = os.path.join(root_path, 'Results Comparison regr')
if not os.path.exists(results_comparison_path):
    os.makedirs(results_comparison_path)
results_comparison_logger_filename = os.path.join(results_comparison_path, 'Best Results Comparison_'+time_string_start+ '.txt')
...
for dataset_name in datasets:
    ...
    #for model_type in ["Ring"]:
        ...
        hypermodel = None
        if(model_type == "ESN"):
            hypermodel = HyperESN_regression()
        elif(model_type == "Ring"):
            hypermodel = HyperRing_regression()
        elif(model_type == "MF-ESN"):
            hypermodel = HyperMF_ESN_regression()
        elif(model_type == "MF-Ring"):
            hypermodel = HyperMF_Ring_regression()
        elif(model_type == "Ring_MinComp"):
             hypermodel = HyperRing_MinComp_regression()
        elif(model_type == "MF-Ring_MinComp"):
             hypermodel = HyperMF_Ring_MinComp_regression()
        ...
        tuner = keras_tuner.BayesianOptimization(
                hypermodel,
                keras_tuner.Objective("val_LSME", direction="min"),
        ...
        print(f"\n[{search_end}] {model_type} on {dataset_name} best results: ")
        tuner.results_summary(1)

        #trovata la combinazione di parametri migliore, procedo all'allenamento e al test
